...
func testCopySchemaShardWithDifferentDB(t *testing.T, shard int) {
	...
	mainTabletAlias := clusterInstance.Keyspaces[0].Shards[shard].Vttablets[0].VttabletProcess.TabletPath
	schema, err := clusterInstance.VtctlclientProcess.ExecuteCommandWithOutput("GetSchema", mainTabletAlias)
	require.Nil(t, err)
	...
	err = clusterInstance.VtctlclientProcess.ExecuteCommand("ExecuteFetchAsDba", "-json", mainTabletAlias, "ALTER DATABASE vt_ks CHARACTER SET latin1")
	require.Nil(t, err)
