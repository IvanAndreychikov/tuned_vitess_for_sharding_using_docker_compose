...
func TestTabletCommands(t *testing.T) {
	...
	sql = "select * from t1"
	result, err = clusterInstance.VtctlclientProcess.ExecuteCommandWithOutput("ExecuteFetchAsDba", "-json", mainTablet.Alias, sql)
	require.Nil(t, err)
	assertExecuteFetch(t, result)

	// check Ping / RefreshState / RefreshStateByShard
	err = clusterInstance.VtctlclientProcess.ExecuteCommand("Ping", mainTablet.Alias)
	require.Nil(t, err, "error should be Nil")

	err = clusterInstance.VtctlclientProcess.ExecuteCommand("RefreshState", mainTablet.Alias)
	require.Nil(t, err, "error should be Nil")
	...
	// Check basic actions.
	err = clusterInstance.VtctlclientProcess.ExecuteCommand("SetReadOnly", mainTablet.Alias)
	require.Nil(t, err, "error should be Nil")
	qr := exec(t, mainConn, "show variables like 'read_only'")
	got := fmt.Sprintf("%v", qr.Rows)
	want := "[[VARCHAR(\"read_only\") VARCHAR(\"ON\")]]"
	assert.Equal(t, want, got)

	err = clusterInstance.VtctlclientProcess.ExecuteCommand("SetReadWrite", mainTablet.Alias)
	require.Nil(t, err, "error should be Nil")
	qr = exec(t, mainConn, "show variables like 'read_only'")
	got = fmt.Sprintf("%v", qr.Rows)
...
func TestActionAndTimeout(t *testing.T) {
	...
	defer cluster.PanicHandler(t)
	err := clusterInstance.VtctlclientProcess.ExecuteCommand("Sleep", mainTablet.Alias, "5s")
	require.Nil(t, err)
	time.Sleep(1 * time.Second)

	// try a frontend RefreshState that should timeout as the tablet is busy running the other one
	err = clusterInstance.VtctlclientProcess.ExecuteCommand("RefreshState", mainTablet.Alias, "-wait-time", "2s")
...
func TestHook(t *testing.T) {
	...
	runHookAndAssert(t, []string{
		"ExecuteHook", mainTablet.Alias, "test.sh", "--flag1", "--param1=hello"}, "0", false, "")

	// test stderr output
	runHookAndAssert(t, []string{
		"ExecuteHook", mainTablet.Alias, "test.sh", "--to-stderr"}, "0", false, "ERR: --to-stderr\n")

	// test commands that fail
	runHookAndAssert(t, []string{
		"ExecuteHook", mainTablet.Alias, "test.sh", "--exit-error"}, "1", false, "ERROR: exit status 1\n")

	// test hook that is not present
	runHookAndAssert(t, []string{
		"ExecuteHook", mainTablet.Alias, "not_here.sh", "--exit-error"}, "-1", false, "missing hook")

	// test hook with invalid name

	runHookAndAssert(t, []string{
		"ExecuteHook", mainTablet.Alias, "/bin/ls"}, "-1", true, "hook name cannot have")
}
...
func TestShardReplicationFix(t *testing.T) {
	// make sure the replica is in the replication graph, 2 nodes: 1 main, 1 replica
	defer cluster.PanicHandler(t)
