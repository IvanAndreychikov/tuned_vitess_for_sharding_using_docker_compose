...
func (mariadbFlavor) restartSubordinateCommands() []string {
	return []string{
		"STOP SLAVE",
		"RESET SLAVE",
		"START SLAVE",
...
func (mariadbFlavor) resetReplicationCommands(c *Conn) []string {
	...
	if c.SemiSyncExtensionLoaded() {
		resetCommands = append(resetCommands, "SET GLOBAL rpl_semi_sync_main_enabled = false, GLOBAL rpl_semi_sync_subordinate_enabled = false") // semi-sync will be enabled if needed when subordinate is started.
	}
