...
func TestInitMainShard(t *testing.T) {
	...
	// wait for shard record to be updated with main
	startTime := time.Now()
	for {
		if time.Since(startTime) > 10*time.Second /* timeout */ {
			si, err := ts.GetShard(ctx, main.Tablet.Keyspace, main.Tablet.Shard)
			if err != nil {
				t.Fatalf("GetShard(%v, %v) failed: %v", main.Tablet.Keyspace, main.Tablet.Shard, err)
			}
			if !topoproto.TabletAliasEqual(si.MainAlias, main.Tablet.Alias) {
				t.Fatalf("ShardInfo should have MainAlias %v but has %v", topoproto.TabletAliasString(main.Tablet.Alias), topoproto.TabletAliasString(si.MainAlias))
			}
		}
		si, err := ts.GetShard(ctx, main.Tablet.Keyspace, main.Tablet.Shard)
		if err != nil {
...
func TestInitMainShardChecks(t *testing.T) {
	...
	if err := wr.InitShardMain(ctx, main2.Tablet.Keyspace, main2.Tablet.Shard, main2.Tablet.Alias, false /*force*/, 10*time.Second); err == nil || !strings.Contains(err.Error(), "is not the shard main") {
