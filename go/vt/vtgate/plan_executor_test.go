...
func TestPlanExecutorAutocommit(t *testing.T) {
	...
	wantSession := &vtgatepb.Session{TargetString: "@main", InTransaction: true, FoundRows: 1}
	...
	wantSession = &vtgatepb.Session{Autocommit: true, TargetString: "@main", FoundRows: 1}
	if !proto.Equal(session.Session, wantSession) {
	...
	wantSession = &vtgatepb.Session{InTransaction: true, Autocommit: true, TargetString: "@main", FoundRows: 1}
	testSession = *session.Session
	testSession.ShardSessions = nil
	if !proto.Equal(&testSession, wantSession) {
