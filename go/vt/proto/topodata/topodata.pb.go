...
type Tablet struct {
	...
	// main_term_start_time is the time (in UTC) at which the current term of
	// the current tablet began as main. If this tablet is not currently the
	// main, this value is ignored.
	//
	// A new main term begins any time an authoritative decision is communicated
	// about which tablet should be the main, such as via Vitess
	// replication-management commands like PlannedReparentShard,
	// EmergencyReparentShard, and TabletExternallyReparented.
	//
	MainTermStartTime  *vttime.Time `protobuf:"bytes,14,opt,name=main_term_start_time,json=mainTermStartTime,proto3" json:"main_term_start_time,omitempty"`
	XXX_NoUnkeyedLiteral struct{}     `json:"-"`
	XXX_unrecognized     []byte       `json:"-"`
	XXX_sizecache        int32        `json:"-"`
}
...
func (m *Tablet) GetMainTermStartTime() *vttime.Time {
	if m != nil {
		return m.MainTermStartTime
	}
	return nil
}
...
type Shard struct {
	// main_alias is the tablet alias of the main for the shard.
	// If it is unset, then there is no main in this shard yet.
	...
	// main_term_start_time is the time (in UTC) at which the current term of
	// the main specified in main_alias began.
	//
	// A new main term begins any time an authoritative decision is communicated
	// about which tablet should be the main, such as via Vitess
	// replication-management commands like PlannedReparentShard,
	// EmergencyReparentShard, and TabletExternallyReparented.
	//
	// The main_alias should only ever be changed if the new main's term began
	// at a later time than this. Note that a new term can start for the tablet
	// that is already the main. In that case, the main_term_start_time would
	// be increased without changing the main_alias.
	MainTermStartTime *vttime.Time `protobuf:"bytes,8,opt,name=main_term_start_time,json=mainTermStartTime,proto3" json:"main_term_start_time,omitempty"`
...
func (m *Shard) GetMainTermStartTime() *vttime.Time {
	if m != nil {
		return m.MainTermStartTime
	}
	return nil
