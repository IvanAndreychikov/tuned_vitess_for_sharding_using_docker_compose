...
func (agent *ActionAgent) TabletExternallyReparented(ctx context.Context, externalID string) error {
	...
	if tablet.Type != topodatapb.TabletType_MASTER {
		...
		newTablet.MainTermStartTime = logutil.TimeToProto(agent.mainTermStartTime())
...
func (agent *ActionAgent) finalizeTabletExternallyReparented(ctx context.Context, si *topo.ShardInfo, ev *events.Reparent) (err error) {
	...
	go func() {
		...
		_, err := topotools.ChangeType(ctx, agent.TopoServer, agent.TabletAlias, topodatapb.TabletType_MASTER, logutil.TimeToProto(agent.mainTermStartTime()))
		if err != nil {
	...
	if !topoproto.TabletAliasIsZero(oldMainAlias) && !topoproto.TabletAliasEqual(oldMainAlias, agent.TabletAlias) {
		...
		go func() {
			...
			var err error
			oldMainTablet, err = topotools.ChangeType(ctx, agent.TopoServer, oldMainAlias, topodatapb.TabletType_REPLICA, nil)
			if err != nil {
				errs.RecordError(err)
				return
			}
