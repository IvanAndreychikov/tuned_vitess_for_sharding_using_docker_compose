...
func TestStateChangeImmediateHealthBroadcast(t *testing.T) {
	...
	for {
		if time.Since(startTime) > 10*time.Second /* timeout */ {
			...
			if !topoproto.TabletAliasEqual(si.MainAlias, agent.Tablet().Alias) {
				t.Fatalf("ShardInfo should have MainAlias %v but has %v", topoproto.TabletAliasString(agent.Tablet().Alias), topoproto.TabletAliasString(si.MainAlias))
			}
		}
		si, err := agent.TopoServer.GetShard(ctx, agent.Tablet().Keyspace, agent.Tablet().Shard)
		if err != nil {
