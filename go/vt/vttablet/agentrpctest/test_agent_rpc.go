...
func (fra *fakeRPCAgent) SetMain(ctx context.Context, parent *topodatapb.TabletAlias, timeCreatedNS int64, waitPosition string, forceStartSubordinate bool) error {
	if fra.panics {
		panic(fmt.Errorf("test-triggered panic"))
	}
	compare(fra.t, "SetMain parent", parent, testMainAlias)
	compare(fra.t, "SetMain timeCreatedNS", timeCreatedNS, testTimeCreatedNS)
	compare(fra.t, "SetMain waitPosition", waitPosition, testWaitPosition)
...
func agentRPCTestSetMain(ctx context.Context, t *testing.T, client tmclient.TabletManagerClient, tablet *topodatapb.Tablet) {
	err := client.SetMain(ctx, tablet, testMainAlias, testTimeCreatedNS, testWaitPosition, testForceStartSubordinate)
