...
func init() {
	...
	addCommand("Shards", command{
		...
		"-keyspace_shard=<keyspace/shard> [-new_main=<tablet alias>] [-avoid_main=<tablet alias>] [-wait_subordinate_timeout=<duration>]",
...
func commandPlannedReparentShard(ctx context.Context, wr *wrangler.Wrangler, subFlags *flag.FlagSet, args []string) error {
	...
	waitSubordinateTimeout := subFlags.Duration("wait_subordinate_timeout", *topo.RemoteOperationTimeout, "time to wait for replicas to catch up on replication before and after reparenting")
