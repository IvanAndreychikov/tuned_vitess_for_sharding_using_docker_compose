...
// RestartSubordinate stops, resets and starts a subordinate.
func (mysqld *Mysqld) RestartSubordinate(hookExtraEnv map[string]string) error {
	h := hook.NewSimpleHook("preflight_stop_subordinate")
	h.ExtraEnv = hookExtraEnv
	if err := h.ExecuteOptional(); err != nil {
		return err
	...
	if err := mysqld.executeSuperQueryListConn(ctx, conn, conn.RestartSubordinateCommands()); err != nil {
		return err
	}
...
func (mysqld *Mysqld) WaitMainPos(ctx context.Context, targetPos mysql.Position) error {
	...
	// If we are the main, WaitUntilPositionCommand will fail.
	// But position is most likely reached. So, check the position
	// first.
	mpos, err := conn.MainPosition()
	if err != nil {
		return fmt.Errorf("WaitMainPos: MainPosition failed: %v", err)
	}
